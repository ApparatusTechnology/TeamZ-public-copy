...
using Assets.GameSaving.Serialization;
...
namespace TeamZ.GameSaving
{
    ...
    public class GameStorage
	{
        ...
        private TypePackSerializer serializer;
        ...
        public GameStorage()
        {
            this.serializer = new TypePackSerializer();
        ...
        public async Task<GameState> LoadAsync(string slotName)
        {
            using (var reader = new FileStream(this.CreateFilePath(slotName), FileMode.Open))
            {
                ...
                var gameState = this.serializer.Deserialize<GameState>(bytes);
        ...
        {
            var bytes = serializer.Serialize(game);
